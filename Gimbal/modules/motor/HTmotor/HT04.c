...
static void HTMotorSetMode(HTMotor_Mode_t cmd, HTMotorInstance *motor) {
  memset(motor->motor_can_instace->tx_buff, 0xff,
         7); // 发送电机指令的时候前面7bytes都是0xff
...
static void HTMotorDecode(CANInstance *motor_can) {
  ...
  measure->speed_rads =
      AverageFilter((uint_to_float(tmp, V_MIN, V_MAX, 12) - HT_SPEED_BIAS),
                    measure->speed_buff, SPEED_BUFFER_SIZE);

  tmp = (uint16_t)(((rxbuff[4] & 0x0f) << 8) | rxbuff[5]);
  measure->real_current =
      HT_CURRENT_SMOOTH_COEF * uint_to_float(tmp, T_MIN, T_MAX, 12) +
...
void HTMotorCalibEncoder(HTMotorInstance *motor) {
  ...
  memcpy(
      zero_buff, buf,
      6); // 初始化的时候至少调用一次,故将其他指令为0时发送的报文保存一下,详见ht04电机说明
  CANTransmit(motor->motor_can_instace, 1);
  DWT_Delay(0.005);
...
HTMotorInstance *HTMotorInit(Motor_Init_Config_s *config) {
  ...
  DWT_Delay(
      0.05f); // 保证下一个电机发送时CAN是空闲的,注意应用在初始化模块的时候不应该进入中断
  ht_motor_instance[idx++] = motor;
  return motor;
}

void HTMotorSetRef(HTMotorInstance *motor, float ref) { motor->pid_ref = ref; }
...
__attribute__((noreturn)) void HTMotorTask(void const *argument) {
  ...
  while (1) {
    ...
    if ((setting->close_loop_type & ANGLE_LOOP) &&
      ...
      pid_ref =
          PIDCalculate(&motor->angle_PID, pid_measure * RAD_2_DEGREE, pid_ref);
    }
    ...
    if ((setting->close_loop_type & SPEED_LOOP) &&
      ...
      pid_ref =
          PIDCalculate(&motor->speed_PID, pid_measure * RAD_2_DEGREE, pid_ref);
    }
...
void HTMotorControlInit() {
  ...
  for (size_t i = 0; i < idx; i++) {
    ...
    ht_task_handle[i] =
        osThreadCreate(osThread(ht_task_name), ht_motor_instance[i]);
  }
}

void HTMotorStop(HTMotorInstance *motor) { motor->stop_flag = MOTOR_STOP; }

void HTMotorEnable(HTMotorInstance *motor) { motor->stop_flag = MOTOR_ENALBED; }
