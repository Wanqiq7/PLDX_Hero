...
void ShootInit() // 已适配四摩擦轮发射机构
{
              ...
              .speed_PID =
                  {
                      ...
                      .IntegralLimit =
                          2000, // ✅ 从1200增大到2000（允许更大积分输出）
                      .Output_LPF_RC =
                          0.02f, // 输出低通滤波时间常数（0.01-0.05，越大越平滑）
  ...
  friction_config.controller_param_init_config.current_feedforward_ptr =
      &fric_ff_cur_lf;
  ...
  fric_ff_dir_lf = (friction_lf->motor_settings.motor_reverse_flag ==
                    MOTOR_DIRECTION_REVERSE)
                       ? -1.0f
                       : 1.0f;
  fric_ff_dir_rf = (friction_rf->motor_settings.motor_reverse_flag ==
                    MOTOR_DIRECTION_REVERSE)
                       ? -1.0f
                       : 1.0f;
  fric_ff_dir_lb = (friction_lb->motor_settings.motor_reverse_flag ==
                    MOTOR_DIRECTION_REVERSE)
                       ? -1.0f
                       : 1.0f;
  fric_ff_dir_rb = (friction_rb->motor_settings.motor_reverse_flag ==
                    MOTOR_DIRECTION_REVERSE)
                       ? -1.0f
                       : 1.0f;
              ...
              .angle_PID =
                  {
                      ...
                      .Kp = 1.65f, // P项（误差1140°→输出2280°/s）
                      .Ki = 1.2f,  // I项消除稳态误差
                      .Kd = 0.0f,
                      .Improve = PID_Integral_Limit,
                      .IntegralLimit = 2500, // 积分限幅
                      .MaxOut = 5000,        // 最大目标速度3000°/s
                  },
              .speed_PID =
                  {
                      // 无电流环模式：速度环直接输出电压指令到电机
                      // 输出单位：电压指令（范围约-16384到+16384）
                      .Kp = 3.25f, // 速度环比例（开环输出需要更大的Kp）
                      .Ki = 0.95f, // 速度环积分（消除稳态误差）
                      .Kd = 0.0f,
                      .Improve = PID_Integral_Limit,
                      .IntegralLimit = 7500, // 积分限幅
                      .MaxOut = 25000,       // 输出限幅（电压指令范围）
                  },
              .current_PID =
                  {
...
void ShootTask() {
  ...
  case LOAD_BURSTFIRE:
    ...
    // REDUCTION_RATIO_LOADER / 8; DJIMotorSetRef(loader, target_speed);
  ...
  if (shoot_cmd_recv.friction_mode == FRICTION_ON) {
    ...
    case SMALL_AMU_15:
      DJIMotorSetRef(friction_lf, 20000); // 15m/s弹速对应转速
      DJIMotorSetRef(friction_lb, 20000);
      DJIMotorSetRef(friction_rb, 20000);
      DJIMotorSetRef(friction_rf, 20000);
      break;
    case SMALL_AMU_18:
      DJIMotorSetRef(friction_lf, 20000); // 18m/s弹速对应转速
      DJIMotorSetRef(friction_lb, 20000);
      DJIMotorSetRef(friction_rb, 20000);
      DJIMotorSetRef(friction_rf, 20000);
      break;
    case SMALL_AMU_30:
      DJIMotorSetRef(friction_lf, 20000); // 30m/s弹速对应转速
      DJIMotorSetRef(friction_lb, 20000);
      DJIMotorSetRef(friction_rb, 20000);
      DJIMotorSetRef(friction_rf, 20000);
      break;
