...
uint8_t CToC_MasterSendControl(void)
{
	...
	{
		if(Remote_RxData.Remote_Key_X==0)TxMessage.Data[5]=3;//遥控器左侧拨动开关
		else TxMessage.Data[5]=2;
	}
...
//	if(i>=0xFFF)return 0;//发送失败
//	return 1;//发送成功
	mbox=CAN_Transmit(CAN1,&TxMessage);//发送数据并获取邮箱号
	i=0;
...
{
	CanTxMsg TxMessage;
	TxMessage.StdId=CToC_SlaveID4;
	TxMessage.RTR=CAN_RTR_Data;//数据帧
	TxMessage.IDE=CAN_Id_Standard;//标准格式
	TxMessage.DLC=0x08;//8字节数据段
	TxMessage.Data[0]=(uint8_t)((uint16_t)(AttitudeAlgorithms_DegPitch*100.0f)>>8);
	TxMessage.Data[1]=(uint8_t)((uint16_t)(AttitudeAlgorithms_DegPitch*100.0f) & 0x00FF);
	TxMessage.Data[2]=(uint8_t)((uint16_t)(AttitudeAlgorithms_ThisDegYaw*100.0f)>>8);
	TxMessage.Data[3]=(uint8_t)((uint16_t)(AttitudeAlgorithms_ThisDegYaw*100.0f) & 0x00FF);
	TxMessage.Data[4]=Remote_RxData.Remote_Key_F;//遥控器启动标志位
	TxMessage.Data[5]=Remote_RxData.Remote_Key_V;//遥控器左侧拨动开关
	TxMessage.Data[6]=0;
	TxMessage.Data[7]=0;
...
{
	CanTxMsg TxMessage;
	TxMessage.StdId=CToC_SlaveID3;
	TxMessage.RTR=CAN_RTR_Data;//数据帧
	TxMessage.IDE=CAN_Id_Standard;//标准格式
	TxMessage.DLC=0x08;//8字节数据段
	TxMessage.Data[0]=RefereeSystem_ShooterStatus;//遥控器连接状态
	TxMessage.Data[1]=RefereeSystem_GameStatus;//遥控器右侧拨动开关
	if((PC_Go==0 && PC_Back==0 && PC_Left==0 && PC_Right==0 && PC_Spin==0 && PC_Pitch==0) && RefereeSystem_Status==0)//遥控器控制
	{
		TxMessage.Data[2]=(uint8_t)((uint16_t)Remote_RxData.Remote_ThumbWheel>>8);//键盘Ctrl状态
		TxMessage.Data[3]=(uint8_t)((uint16_t)Remote_RxData.Remote_ThumbWheel & 0x00FF);//键盘Shift状态
		TxMessage.Data[4]=0;//遥控器启动标志位
		TxMessage.Data[5]=0;//遥控器左侧拨动开关
		TxMessage.Data[6]=0;
		TxMessage.Data[7]=0;
	}
	else//键盘控制
	{
		if(Remote_RxData.Remote_Mouse_KeyL)
		{
			Fire_Flag=0;
			TxMessage.Data[2]=(uint8_t)((uint16_t)0x0000>>8);//键盘Ctrl状态
			TxMessage.Data[3]=(uint8_t)((uint16_t)0x0000 & 0x00FF);//键盘Shift状态
		}
		else
		{
			TxMessage.Data[2]=(uint8_t)((uint16_t)0x0400>>8);//键盘Ctrl状态
			TxMessage.Data[3]=(uint8_t)((uint16_t)0x0400 & 0x00FF);//键盘Shift状态
		}
		TxMessage.Data[4]=0;//遥控器启动标志位
		TxMessage.Data[5]=0;//遥控器左侧拨动开关
		TxMessage.Data[6]=0;
...
void CToC_CANDataProcess(uint32_t ID,uint8_t *Data)
{
	...
	if(ID==CToC_MasterID1)//接收遥控器拨杆数据
	{
		...
		if(Last_ShooterStatus==1 && RefereeSystem_ShooterStatus==0){RefereeSystem_ShooterCloseFlag=1;RefereeSystem_ShooterCloseCounter=0;}
		
		RefereeSystem_RobotID=Data[1];
		RefereeSystem_GameStatus=Data[2];
	}
}
