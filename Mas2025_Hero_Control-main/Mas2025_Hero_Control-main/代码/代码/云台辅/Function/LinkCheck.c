...
void TIM8_UP_TIM13_IRQHandler(void)
{
	if(TIM_GetITStatus(TIM13,TIM_IT_Update)==SET)//检测TIM2更新
	{
		TIM_ClearITPendingBit(TIM13,TIM_IT_Update);//清除标志位
		{
			...
			LinkCheck_Error=1;
//			if(RefereeSystem_ShooterStatus==0 && RefereeSystem_ShooterCloseFlag==1)//发射机构未上电
//			{
//				CAN_CAN1DeviceNumber=2;//隔离发射机构设备
//				CAN_CAN2DeviceNumber=0;//隔离发射机构设备
//				CAN_DeviceNumber=2;
//				CAN_CAN_GetRefereeSystemData();//复位CAN接收ID列表
//				LinkCheck_ErrorID=-1;
//				LinkCheck_Error=0;
//				Warming_BuzzerClean();
//				
//				RefereeSystem_ShooterCloseCounter++;
//				if(RefereeSystem_ShooterCloseCounter>0)RefereeSystem_ShooterCloseFlag=0;
//			}
//			else//发射机构正常上电
//			{
//				if(RefereeSystem_ShooterStatus==0)CAN_CAN_GetRefereeSystemData();
//				else CAN_CAN_GetRefereeSystemData();//读取裁判系统状态，保证板间裁判系统数据传递不会断开
//				LinkError_Count++;
//				
//				if(LinkError_Count>15)LinkCheck_Error=1;//连接错误
//				else LinkCheck_ErrorID=-1;
//			}
		}
	}
}
