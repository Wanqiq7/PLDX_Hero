 ...
 *
 * @note
 * f407的bxCAN有28个过滤器,这里将其配置为前14个过滤器给CAN1使用,后14个被CAN2使用
...
static void CANAddFilter(CANInstance *_instance) {
  ...
  static uint8_t can1_filter_idx = 0,
                 can2_filter_idx = 14; // 0-13给can1用,14-27给can2用

  can_filter_conf.FilterMode =
      CAN_FILTERMODE_IDLIST; // 使用id
                             // list模式,即只有将rxid添加到过滤器中才会接收到,其他报文会被过滤
  can_filter_conf.FilterScale =
      CAN_FILTERSCALE_16BIT; // 使用16位id模式,即只有低16位有效
...
uint8_t CANTransmit(CANInstance *_instance, float timeout) {
  ...
  while (HAL_CAN_GetTxMailboxesFreeLevel(_instance->can_handle) ==
         0) // 等待邮箱空闲
  {
    if (DWT_GetTimeline_ms() - dwt_start > timeout) // 超时
    {
      LOGWARNING(
          "[bsp_can] CAN MAILbox full! failed to add msg to mailbox. Cnt [%d]",
          busy_count);
      busy_count++;
      return 0;
    }
 ...
 *
 * @param _hcan
 * @param fifox passed to HAL_CAN_GetRxMessage() to get mesg from a specific
 * fifo
 */
static void CANFIFOxCallback(CAN_HandleTypeDef *_hcan, uint32_t fifox) {
  ...
  {
    HAL_CAN_GetRxMessage(_hcan, fifox, &rxconf,
    ...
    for (size_t i = 0; i < idx; ++i) { // 两者相等说明这是要找的实例
      if (_hcan == can_instance[i]->can_handle &&
        ...
        if (can_instance[i]->can_module_callback !=
            NULL) // 回调函数不为空就调用
        {
          can_instance[i]->rx_len = rxconf.DLC; // 保存接收到的数据长度
          memcpy(can_instance[i]->rx_buff, can_rx_buff,
                 rxconf.DLC); // 消息拷贝到对应实例
          can_instance[i]->can_module_callback(
              can_instance[i]); // 触发回调进行数据解析和处理
        }
        return;
      }
    }
 ...
 *
 * @param hcan CAN handle indicate which device the oddest mesg in FIFO_0 comes
 * from
 */
 ...
 *
 * @param hcan CAN handle indicate which device the oddest mesg in FIFO_1 comes
 * from
 */
